...
// Deterministic per-question correctness for each seeded student (index-aligned with userDefs).
// Pattern index = student index, question index mod 6. Produces a realistic spread incl. fails.
const answerPatterns: boolean[][] = [
...
function readBackendEnv(): Record<string, string> {
  ...
  try {
    ...
    // .env not present; fall back to defaults below.
  }
  return env;
}

async function upsertQuestion(qDef: QuestionDef, subjectId: string, createdById: string): Promise<string> {
  const existing = await prisma.question.findFirst({
    where: { subjectId, prompt: qDef.prompt, tenantId: null },
  });
  ...
  if (existing) {
    ...
    for (const [oi, opt] of qDef.options.entries()) {
      await prisma.questionOption.upsert({
        where: { questionId_label: { questionId: existing.id, label: opt.label } },
        update: { text: opt.text, isCorrect: opt.isCorrect, sortOrder: oi },
        create: { questionId: existing.id, label: opt.label, text: opt.text, isCorrect: opt.isCorrect, sortOrder: oi },
      });
    }
    return existing.id;
  }
...
async function main() {
  ...
  // Create questions (idempotent: matched by subject + prompt)
  ...
  // Create one Question Bank per course and link its seeded questions to it
  const bankRecords: Record<string, string> = {};
  for (const [courseCode, qDefs] of Object.entries(questionsByCourse)) {
    const courseDef = courses.find(c => c.code === courseCode)!;
    const subjectCode = courseDef.subjectCode;
    const courseId = courseRecords[courseCode];
    const categoryId = subjectRecords[subjectCode];
    const bankName = `${courseDef.name} Question Bank`;

    const existingBank = await prisma.questionBank.findFirst({
      where: { name: bankName, courseId, categoryId, tenantId: null },
    });
    ...
    bankRecords[courseCode] = bank.id;

    for (let i = 0; i < qDefs.length; i++) {
      await prisma.question.update({
        where: { id: questionRecords[`${courseCode}_q${i}`] },
        data: { questionBankId: bank.id, sortOrder: i },
      });
    }
  }
  ...
  const examRecords: Array<{ id: string; title: string; totalMarks: number; passingMarks: number }> = [];
  for (const def of examDefs) {
    const exam = await prisma.exam.upsert({
      ...
      update: {
        ...
        endsAt: def.endsAt,
        status: def.status,
        questionBankId: bankRecords[def.courseCode],
      },
      create: {
        courseId: courseRecords[def.courseCode],
        createdById: instructorId,
    ...
    // Ensure exam questions exist with marks that sum to totalMarks
    const base = Math.floor(def.totalMarks / def.questionKeys.length);
    for (let i = 0; i < def.questionKeys.length; i++) {
      const points = i === def.questionKeys.length - 1
        ? def.totalMarks - base * (def.questionKeys.length - 1)
        : base;
      await prisma.examQuestion.upsert({
        where: { examId_questionId: { examId: exam.id, questionId: questionRecords[def.questionKeys[i]] } },
        update: { points, sortOrder: i },
        create: { examId: exam.id, questionId: questionRecords[def.questionKeys[i]], points, sortOrder: i },
      });
    }

    examRecords.push({ id: exam.id, title: exam.title, totalMarks: def.totalMarks, passingMarks: def.passingMarks });
  }
  ...
  const publishableExams = examDefs.filter((_, i) =>
    examDefs[i].status === ExamStatus.PUBLISHED
  );
  for (const def of publishableExams) {
    const examId = examRecords.find(e => e.title === def.title)!.id;
  ...
  for (let ei = 0; ei < closedExams.length; ei++) {
    ...
    const examRec = examRecords.find(e => e.title === examDef.title)!;

    const examQuestions = await prisma.examQuestion.findMany({
      where: { examId: examRec.id },
    ...
    for (let si = 0; si < studentIds.length; si++) {
      ...
      const pattern = answerPatterns[si % answerPatterns.length];

      const session = await prisma.examSession.upsert({
        where: {
          examId_studentId_attemptNumber: {
            examId: examRec.id, studentId, attemptNumber: 1,
          },
        },
        update: {},
        create: {
          ...
          submittedAt: new Date(examDef.startsAt.getTime() + dayMs + examDef.durationMinutes * 60 * 1000 - 3 * 60 * 1000),
          remainingSeconds: 0,
          questionOrder: JSON.stringify(examQuestions.map(eq => eq.questionId)),
          optionOrder: JSON.stringify({}),
        },
      });

      // Create real answers and compute the score from the actual exam marks
      let totalScore = 0;
      for (let qi = 0; qi < examQuestions.length; qi++) {
        const eq = examQuestions[qi];
        const correct = pattern[qi % pattern.length];
        const correctOptions = eq.question.options.filter(o => o.isCorrect);
        const wrongOptions = eq.question.options.filter(o => !o.isCorrect);
        const selectedOptionIds = correct
          ? correctOptions.map(o => o.id)
          : (wrongOptions[0] ? [wrongOptions[0].id] : []);
        const points = Number(eq.points);
        if (correct) totalScore += points;

        await prisma.studentAnswer.upsert({
          where: { sessionId_questionId: { sessionId: session.id, questionId: eq.questionId } },
          update: { selectedOptionIds: JSON.stringify(selectedOptionIds), score: correct ? points : 0 },
          create: {
      ...
      if (percentage >= 80) {
        const existingResult = await prisma.result.findUnique({ where: { submissionId: submission.id } });
        if (existingResult) {
          const certNo = `CERT-${String(ei + 1).padStart(3, '0')}-${String(si + 1).padStart(3, '0')}`;
          const existingCert = await prisma.certificate.findFirst({ where: { resultId: existingResult.id } });
          if (!existingCert) {
            await prisma.certificate.create({
              data: {
                resultId: existingResult.id,
                certificateNo: certNo,
                verificationCode: `VERIFY-${certNo}-${existingResult.id.slice(0, 8)}`,
                issuedAt: new Date(),
              },
            });
          }
      ...
      // Notifications for results (deduplicated by message)
      const message = `Your result for "${examDef.title}" is now available. Score: ${totalScore}/${maxScore} (${percentage}%)`;
      const existingNotif = await prisma.notification.findFirst({
        where: { userId: studentId, type: NotificationType.RESULT_PUBLISHED, message },
      });
      if (!existingNotif) {
  ...
  for (const s of violationSessions) {
    const existingViolation = await prisma.examViolation.findFirst({ where: { sessionId: s.id } });
  ...
  const platformSettings = [
    { key: 'ENVIRONMENT', value: backendEnv['NODE_ENV'] ?? 'development', isPublic: true },
    { key: 'JWT_ACCESS_EXPIRES_IN', value: backendEnv['JWT_ACCESS_EXPIRES_IN'] ?? '15m', isPublic: true },
    { key: 'JWT_REFRESH_EXPIRES_IN', value: backendEnv['JWT_REFRESH_EXPIRES_IN'] ?? '7d', isPublic: true },
    { key: 'RATE_LIMIT_TTL', value: backendEnv['RATE_LIMIT_TTL'] ?? '60', isPublic: false },
    { key: 'RATE_LIMIT_LIMIT', value: backendEnv['RATE_LIMIT_LIMIT'] ?? '120', isPublic: true },
  ];

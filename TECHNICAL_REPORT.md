...
Migration: `backend/prisma/migrations/20260908000000_add_exam_policies_requests/migration.sql` (applied, `finished_at` set, existing data intact — defaults only).
...
- **`exams`**: `connectionLossPolicy` (default `AUTO_RESUME`), `resumePolicy` (default `STUDENT`), `retakePolicy` (default `DISABLED`).
...
| `exam-sessions/exam-sessions.service.ts` | Policy enforcement: `assertResumeAllowed` coded LOCKED errors; retake blocked unless permitted policy level + approved request; `startExam` `RETAKE_REQUIRED`/`RETAKE_PENDING` |
...
| `monitoring/dto/update-monitoring-config.dto.ts`, `monitoring/monitoring.service.ts` | New config fields; enum casts on save; `setConnection` END_SESSION/MARK_REVIEW/APPROVAL_REQUIRED semantics; `assertCanMonitorExam`, `getSessionExamId` helpers |
...
| `features/exams/exam-taking-client.tsx` | Proctoring status banner (starting / active / denied / unavailable + Retry); consent card shows enforced policy (`fullscreen REQUIRED`, strictness) and auto-requests fullscreen on consent; retake UI on `RETAKE_REQUIRED`/`RETAKE_PENDING`; "Request instructor approval now" button on `RESUME_PENDING` |
| `app/(dashboard)/instructor/exams/create/page.tsx` | `connectionLossPolicy` / `resumePolicy` / `retakePolicy` selects |
| `app/(dashboard)/instructor/exams/manage/page.tsx` | Same policy selects wired into edit payload |
...
2. Backend `DATABASE_URL` (pooled) + `DIRECT_DATABASE_URL` (Prisma Migrate) as already configured. Deploy the migration before starting the new backend build.

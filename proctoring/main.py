...
def _cleanup_rate_limits() -> None:
    now = time.time()
    stale = [k for k, v in _rate_limits.items() if not v or now - v[-1] > RATE_LIMIT_WINDOW_SECONDS]
    for k in stale:
...
def _check_rate_limit(client_ip: str) -> bool:
    ...
    with _rate_lock:
        ...
        _rate_limits[client_ip] = [t for t in timestamps if now - t < RATE_LIMIT_WINDOW_SECONDS]
...
app = FastAPI(title="Proctoring Signal Service", version="1.2.0", lifespan=lifespan)
